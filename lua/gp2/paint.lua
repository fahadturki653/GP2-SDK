...
function PaintManager.GetSurfacesInRadius(center, radius)
    ...
    -- Check the cache
    if cache[cacheKey] and currentTime - cache[cacheKey].time < cacheLifetime then
        return cache[cacheKey].surfaces
    end
    ...
    for _, entity in ipairs(entities) do
        ...
        for s = 1, #surfinfo do
            ...
            if IsPointInShape(hitPos, vertices, surfinfo[s]:GetNormal(), radius) then
                table.insert(surfacesInRadius, surfinfo[s])
            end
        end
    end
